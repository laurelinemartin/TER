...
int *Algo_tabou_roulette(int *Horaires, int N, int sommet_depart, int *couleur, int *Type, int **TO, int Nb_iterations, int heure_max, int* Nb_eleves)
{
	...
	for(int k = 0; k < Nb_iterations; k++)
	{
		...
		for(int i = 0; i < N; i++)
		{
			...
			//2 4 8 15 16 17 17.
			if(i!=0) { Valeurs_roulette[i] = congestion_initiale - Meilleure_congestion[i] + Valeurs_roulette[i-1]; }
			else { Valeurs_roulette[i] = congestion_initiale - Meilleure_congestion[i]; }
		}
		printf("Valeurs roulette\n");
		for (int n = 0; n < N; n++) { printf("%d ",Valeurs_roulette[n]); }
		printf("\n");
		...
		if ((choix_roulette >= 0) && (choix_roulette <= Valeurs_roulette[0])) { Horaires_tabou_roulette[0] = Meilleure_solution[0]; Sommets_interdits[k] = 0; printf("Le sommet modifié au final dans l'algo roulette est %d à l'horaire : %d\n",0,Horaires_tabou_roulette[0]);}
		else
		{
			for (int m = 1; m < N; m++)
			{
